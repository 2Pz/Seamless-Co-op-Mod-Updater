...
class AppUpdater(QThread):
    ...
    def __init__(self, localization, silent=False):
        super().__init__()
        ...
        try:
            ...
            if latest_version > self.current_version:
                ...
                if self.should_update:
                    # Always show download progress regardless of silent mode
            ...
            if KeyError:
                self.update_complete.emit(False, self.Localization.translate("messages.errors.Api_limit").format(str(e)))
                return
            if not self.silent:
    ...
    def perform_update(self, latest_release, latest_version):
        try:
            self.update_progress.emit(self.Localization.translate("messages.update.new_version_downloading").format(latest_version))
            asset = next(asset for asset in latest_release['assets'] if asset['name'] == 'SeamlessCo-opManager.zip')
