...
class UpdateThread(QThread):
    ...
    def __init__(self, url, install_path, settings, localization):
    ...
    def run(self):
        try:
            ...
            self.update_progress.emit(self.Localization.translate("messages.update.downloading").format(release_name))
            ...
            self.update_progress.emit(self.Localization.translate("messages.update.extracting"))
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                zip_ref.extractall(self.install_path)

            self.update_progress.emit(self.Localization.translate("messages.update.updating_settings"))
            ...
            os.remove(zip_path)
            self.update_complete.emit(True, self.Localization.translate("messages.update.complete").format(release_name))
